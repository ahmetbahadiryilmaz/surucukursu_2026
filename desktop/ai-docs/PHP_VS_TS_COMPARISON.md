...
| 29 | **ek4 report** | Generates `ek4.pdf` per student (sinav sonuc raporu) | Generated alongside every simulator PDF in `generateEk4Html()` (mebbis-manager.ts) | ✅ FIXED — uses original 910-line PHP template, fills `.kursiyer/.plakano/.egitmen/.companyName/.tarih`, randomizes one of 4 `.sep` groups by toggling the `hidden` class |
...
### ek4
- Server: `${API_BASE_URL}/templates/ek4/ek4.html` (served by `desktop-service` `TemplatesController.getEk4Template`)
- Source on disk: `backend/storage/templates/ek4/ek4.html` (910 lines, copy of original PHP template)
- CSS selectors filled by `generateEk4Html()`: `.kursiyer`, `.plakano`, `.egitmen`, `.companyName` (uppercased), `.tarih` — works on any tag (`<td>`, `<span>`, etc.)
- `.sep1`–`.sep13` — template ships with `hidden` class on all of them; `generateEk4Html()` re-applies `hidden` to all (idempotent), then removes `hidden` from one randomly chosen group: `[1,2,3]`, `[4,5,6]`, `[7,8,9]`, or `[10,11,12,13]`. Visibility is controlled by the template's own `.hidden { display:none; }` rule.
- Generated automatically alongside every simulator report (single + batch flows) and saved as `ek4.pdf` in the student's output folder.
