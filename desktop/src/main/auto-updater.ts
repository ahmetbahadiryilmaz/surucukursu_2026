...
/**
 * Returns changelog lines for the current version if the user hasn't seen them yet.
 * Returns null if already seen (or no changelog entry for this version).
 */
export function getPendingWhatsNew(): { version: string; lines: string[] } | null {
  const current = app.getVersion();
  const lastSeen = readLastSeenVersion();
  if (lastSeen === current) return null;
  const entries = CHANGELOG[current];
  if (!entries || entries.length === 0) return null;
  return { version: current, lines: entries };
}

/** Mark current version as seen so the dialog won't show again. */
export function markWhatsNewSeen(): void {
  writeLastSeenVersion(app.getVersion());
}
...
export function showWhatsNewIfUpdated(mainWindow: BrowserWindow): void {
  ...
  // Prefer server-provided whatsNew over hardcoded CHANGELOG
  const messageBody = _serverWhatsNew || changeLines;
...
interface VersionCheckResult {
   ...
   * 'below-min'    — version is below minimumVersion  (auto-updater can fix)
   * 'above-max'    — version exceeds maximumVersion   (cannot auto-downgrade)
   * 'no-check'     — strictVersionCheck is false, skipped entirely
   */
  reason: 'ok' | 'below-min' | 'above-max' | 'no-check';
...
/** Stored from the last successful version check; used by showWhatsNewIfUpdated */
let _serverWhatsNew: string | null = null;
...
export async function enforceVersionCheckWithSplash(): Promise<boolean> {
  ...
  try {
    ...
    if (result.allowed) {
      ...
      // Store whatsNew from server for use by showWhatsNewIfUpdated
      if (result.whatsNew) _serverWhatsNew = result.whatsNew;
    ...
    // VERSION BLOCKED
    console.log(
      `[VersionGate] BLOCKED! Current: ${APP_VERSION}, reason: ${result.reason}, min: ${result.minimumVersion}, max: ${result.maximumVersion}`,
    );

    if (result.reason === 'above-max') {
      // Cannot auto-downgrade — show error and quit
      setStatus('Sürüm uyumsuz!');
      await dialog.showMessageBox(splash, {
        type: 'error',
        title: 'Sürüm Uyumsuz',
        message:
          `Bu sürüm (v${APP_VERSION}) bu sistem için fazla yeni.\n` +
          `İzin verilen maksimum sürüm: v${result.maximumVersion}\n\n` +
    ...
    // below-min — force mandatory update in splash window
...
export function setupAutoUpdater(mainWindow: BrowserWindow | null) {
  ...
  periodicCheckInterval = setInterval(async () => {
    try {
      const result = await fetchVersionCheck(APP_VERSION);
      if (!result.allowed) {
        console.log(`[PeriodicVersionCheck] BLOCKED! Current: ${APP_VERSION}, reason: ${result.reason}, min: ${result.minimumVersion}, max: ${result.maximumVersion}`);
        // Stop further checks
...
/**
 * Fetch version config from minimum_version.json and compare with current app version.
 *
 * Supports:
 *   minimumVersion     — block if app is below this (mandatory update)
...
function fetchVersionCheck(
  ...
  return new Promise((resolve, reject) => {
    const transport = VERSION_CHECK_URL.startsWith('https') ? https : http;
      ...
      (res) => {
        ...
        res.on('end', () => {
          try {
            ...
            // If strictVersionCheck is explicitly false, skip all version blocking.
            // Useful during maintenance, rollback, or for users exempt from version requirements.
            const strictVersionCheck = json.strictVersionCheck !== false; // default: true
            if (!strictVersionCheck) {
              resolve({
                allowed: true,
                reason: 'no-check',
                latestVersion: json.minimumVersion || version,
                minimumVersion: json.minimumVersion || '0.0.0',
                maximumVersion: json.maximumVersion || null,
                message: 'Sürüm kontrolü devre dışı.',
                whatsNew: json.whatsNew || undefined,
              });
              return;
            }
            ...
            if (minimumVersion && !/^\d+\.\d+\.\d+/.test(minimumVersion)) {
              reject(new Error('Invalid minimumVersion format in minimum_version.json'));
              return;
            }

            const maximumVersion: string | null = json.maximumVersion || null;
            if (maximumVersion && !/^\d+\.\d+\.\d+/.test(maximumVersion)) {
              reject(new Error('Invalid maximumVersion format in minimum_version.json'));
              return;
            }

            let allowed = true;
            let reason: VersionCheckResult['reason'] = 'ok';

            if (minimumVersion && compareSemver(version, minimumVersion) < 0) {
              allowed = false;
              reason = 'below-min';
            } else if (maximumVersion && compareSemver(version, maximumVersion) > 0) {
              allowed = false;
              reason = 'above-max';
            }
            ...
            resolve({
              allowed,
              reason,
              latestVersion: minimumVersion || version,
              minimumVersion: minimumVersion || '0.0.0',
              maximumVersion,
              message: json.message || (allowed
                ? 'Sürümünüz güncel.'
                : reason === 'above-max'
                  ? `Bu sürüm (v${version}) izin verilen maksimum sürümü (v${maximumVersion}) aşıyor.`
