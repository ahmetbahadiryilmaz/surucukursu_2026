...
function setupIPC() {
  ...
  ipcMain.handle('auth:check', async () => {
    ...
    try {
      const school = await apiClient.getMySchool(token);
      return { user, school };
    } catch {
      authStore.clear();
      return null;
  ...
  ipcMain.handle('auth:login', async (_event, email: string, password: string) => {
    ...
    const school = await apiClient.getMySchool(result.token);
  ...
  ipcMain.handle('auth:forgot-password', async (_event, email: string, phone: string) => {
    return apiClient.forgotPassword(email, phone);
  });
  ...
  ipcMain.handle('auth:reset-password', async (_event, email: string, code: string, newPassword: string) => {
    return apiClient.resetPassword(email, code, newPassword);
  });
