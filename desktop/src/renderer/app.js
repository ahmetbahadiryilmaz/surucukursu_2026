...
function showMain(schoolName) {
  loginView.style.display  = 'none';
  forgotView.style.display = 'none';
  mainView.style.display   = 'block';
  const badge = document.getElementById('school-name');
  if (badge) badge.textContent = schoolName || '';
}
...
document.getElementById('forgot-form-step2').addEventListener('submit', async (e) => {
  ...
  try {
    ...
    errorEl.textContent   = err?.message || 'Geçersiz kod veya bir hata oluştu.';
    errorEl.style.display = 'block';
    btn.disabled    = false;
    btn.textContent = 'Şifremi Sıfırla';
  }
...
function openModal(editAccount) {
  ...
  if (editAccount) {
    ...
    if (sub) {
      const typeMap = { paid: 'Ücretli', demo: 'Demo' };
