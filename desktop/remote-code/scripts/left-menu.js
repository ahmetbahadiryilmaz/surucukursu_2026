"use strict";
(() => {
  // src/ui/mebbis-left-menu/modals/direksiyon.js
  function showDireksiyonModal() {
    ...
    sinifSelect.onfocus = () => {
      sinifSelect.style.borderColor = "#4361ee";
    };
    sinifSelect.onblur = () => {
      sinifSelect.style.borderColor = "#2a2a4a";
    };
    const sinifOptions = [
  ...
  // src/ui/mebbis-left-menu/modals/simulasyon.js
  function showSimulasyonModal() {
    ...
    tcInput.onfocus = () => {
      tcInput.style.borderColor = "#4361ee";
    };
    tcInput.onblur = () => {
      tcInput.style.borderColor = "#2a2a4a";
    };
  ...
  // src/ui/mebbis-left-menu/k-belgesi/gate.js
  function showKBGate(missingKind) {
    var gateOv = document.createElement("div");
    gateOv.id = "mebbis-modal-overlay";
    gateOv.style.cssText = "position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.6); z-index: 99999; display: flex; align-items: center; justify-content: center;";
    var gateModal = document.createElement("div");
    gateModal.style.cssText = "background: #1a1a2e; border: 1px solid #2a2a4a; border-radius: 10px; padding: 24px 28px; width: 400px; font-family: Arial, sans-serif; color: white; text-align: center;";
    ...
    gateMsg.textContent = missingKind === "kurum" ? "L\xFCtfen Kurum Bilgisi g\xFCncelleyin." : "L\xFCtfen Personel Bilgisi g\xFCncelleyin.";
  ...
  // src/ui/mebbis-left-menu/k-belgesi/search.js
  function showKBSearch(overlay, students, onSelect) {
    var searchModal = document.createElement("div");
    searchModal.style.cssText = "background: #1a1a2e; border: 1px solid #2a2a4a; border-radius: 10px; padding: 24px 28px; width: 520px; font-family: Arial, sans-serif; color: white; margin: auto;";
    ...
    hint.textContent = students.length ? "TC veya Ad Soyad ile aray\u0131n (" + students.length + " kay\u0131tl\u0131 \xF6\u011Frenci)" : "Hen\xFCz \xF6\u011Frenci y\xFCklenmemi\u015F \u2014 \xF6nce \xD6\u011Frencileri G\xFCncelle. Manuel TC ile devam edebilirsiniz.";
    searchModal.appendChild(hint);
    var searchInput = document.createElement("input");
    searchInput.type = "text";
    searchInput.placeholder = "TC veya Ad Soyad";
    searchInput.style.cssText = "width: 100%; padding: 10px; border: 1px solid #2a2a4a; border-radius: 4px; background: #16213e; color: white; font-size: 14px; box-sizing: border-box; outline: none; margin-bottom: 8px;";
    searchInput.onfocus = function() {
      searchInput.style.borderColor = "#4361ee";
    };
    searchInput.onblur = function() {
      searchInput.style.borderColor = "#2a2a4a";
    };
    ...
    function renderResults(q) {
      ...
      matches.forEach(function(s) {
        ...
        left.innerHTML = '<div style="color:white;">' + (s.adSoyad || "-") + '</div><div style="color:#888; font-size:11px;">TC: ' + (s.tc || "-") + (s.grubu ? " \xB7 " + s.grubu : "") + "</div>";
  ...
  // src/ui/mebbis-left-menu/k-belgesi/form.js
  function buildKBForm(modal, student, personnelList, store, overlay) {
    ...
    function mkField(parent, label, id, opts) {
      ...
      if (opts.type === "select") {
        ...
        (opts.options || []).forEach(function(o) {
          ...
          op.value = o.value !== void 0 ? o.value : o;
          op.textContent = o.label !== void 0 ? o.label : o;
      ...
      inp.onfocus = function() {
        inp.style.borderColor = "#4361ee";
      };
      inp.onblur = function() {
        inp.style.borderColor = "#2a2a4a";
      };
    ...
    if (student && Array.isArray(student.lessons)) {
      for (var li = 0; li < student.lessons.length; li++) {
        if (student.lessons[li] && student.lessons[li].dersYeri) {
          lessonDersYeri = student.lessons[li].dersYeri;
          break;
        }
      }
    }
    ...
    if (autoPickedPersonnelTc) {
      for (var pi = 0; pi < personnelList.length; pi++) {
        if (personnelList[pi] && personnelList[pi].tc === autoPickedPersonnelTc) {
          autoPersonnel = personnelList[pi];
          break;
        }
      }
    }
    ...
    if (!personnelKurum || !personnelIl) {
      for (var pfi = 0; pfi < personnelList.length; pfi++) {
        ...
        if (!pf)
          continue;
        if (!personnelKurum && pf.kurumAdi)
          personnelKurum = pf.kurumAdi;
        if (!personnelIl && pf.il)
          personnelIl = pf.il;
        if (!personnelIlce && pf.ilce)
          personnelIlce = pf.ilce;
        if (personnelKurum && personnelIl && personnelIlce)
          break;
      }
    }
    ...
    if (Array.isArray(store.cars)) {
      store.cars.forEach(function(c) {
        if (c.plate_number)
          carsIndex[c.plate_number.toUpperCase()] = c;
      });
    }
    var matchedCar = null;
    if (student && Array.isArray(student.lessons)) {
      for (var ci = 0; ci < student.lessons.length; ci++) {
        ...
        if (lp && carsIndex[lp.toUpperCase()]) {
          matchedCar = carsIndex[lp.toUpperCase()];
          break;
        }
      }
    }
    ...
    var kursAdiRaw = kurumInfo && (kurumInfo.kurum_adi || kurumInfo.kurumAdi) || personnelKurum || student && student.kurum || "";
    ...
    var saveRouteIfChanged = function() {
      ...
      if (!val || val === lastSavedKurumRoute)
        return;
      lastSavedKurumRoute = val;
      if (kurumInfo)
        kurumInfo.kurum_route = val;
    ...
    var savePersonalIfChanged = function() {
      ...
      ["babaAd", "dogumYeri", "dogumTarihi", "adres"].forEach(function(k) {
        if (current[k] !== lastSavedPersonal[k]) {
          diff[k] = current[k];
          changed = true;
        }
      });
      ...
      console.log("MEBBIS_SAVE_STUDENT_PERSONAL:" + JSON.stringify(Object.assign({ tc }, diff)));
    };
    ...
    personnelList.forEach(function(p2) {
      if (!p2 || !p2.tc)
        return;
      personnelOpts.push({ value: p2.tc, label: (p2.adSoyad || (p2.ad || "") + " " + (p2.soyad || "")) + " \xB7 " + p2.tc });
    });
    ...
    submitBtn.onclick = function() {
      ...
      if (adayTcVal.length !== 11 || !/^[0-9]+$/.test(adayTcVal))
        errors.push("adayTc");
      if (ustaTcVal.length !== 11 || !/^[0-9]+$/.test(ustaTcVal))
        errors.push("ustaTc");
  ...
  // src/ui/mebbis-left-menu/k-belgesi/index.js
  function handleKBelgesi(store) {
    ...
    overlay.id = "mebbis-modal-overlay";
    overlay.style.cssText = "position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.7); z-index: 99999; display: flex; align-items: flex-start; justify-content: center; overflow-y: auto; padding: 20px 0;";
    var modal = document.createElement("div");
    modal.style.cssText = "background: #1a1a2e; border: 1px solid #2a2a4a; border-radius: 10px; padding: 24px 28px; width: 600px; font-family: Arial, sans-serif; color: white; margin: auto;";
  ...
  // src/ui/mebbis-left-menu/index.js
