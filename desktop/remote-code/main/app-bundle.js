...
var IS_DEV = !import_electron2.app.isPackaged;
function loadDevEnv() {
  if (!IS_DEV)
    return {};
  const projectRoot = path2.resolve(__dirname, "..", "..");
  const candidates = [".env.local", ".env"].map((f) => path2.join(projectRoot, f));
  const out = {};
...
var devEnv = loadDevEnv();
function envOr(key, fallback) {
  return process.env[key] || devEnv[key] || fallback;
}
var PROD_API_BASE = "https://api.mtsk.app";
var PROD_UPDATES_BASE = "https://mtsk.app";
var DEV_DEFAULT_API_BASE = "http://127.0.0.1:9501";
var API_BASE_URL = IS_DEV ? envOr("DESKTOP_API_BASE_URL", DEV_DEFAULT_API_BASE) : PROD_API_BASE;
var UPDATES_BASE_URL = IS_DEV ? envOr("DESKTOP_UPDATES_BASE_URL", API_BASE_URL) : PROD_UPDATES_BASE;
var DESKTOP_KEY_HEX = "4a391941f894909f4d734e928468af71db7ef39576728068f5be8a4499472bab";
var DESKTOP_HMAC_SECRET = "b8c44c162f5891a1b5fcb079fcb42189042f4a3c230906d57c361d5feedcb8d7";
var ENCRYPTED_TEMPLATE_URL = `${API_BASE_URL}/desktop/desktop-service/templates/encrypted`;
var DESKTOP_CODE_BASE_URL = `${API_BASE_URL}/desktop/desktop-service/desktop-code`;
var FORCE_REMOTE_CODE_IN_DEV = /^(1|true|yes)$/i.test(envOr("DESKTOP_FORCE_REMOTE_CODE", ""));
var FORCE_VERSION_CHECK_IN_DEV = /^(1|true|yes)$/i.test(envOr("DESKTOP_FORCE_VERSION_CHECK", ""));
var OPEN_DEVTOOLS_IN_DEV = /^(1|true|yes)$/i.test(envOr("DESKTOP_OPEN_DEVTOOLS", ""));
var VERSION_POLL_SECONDS = (() => {
...
var apiClient = {
  ...
  updateStudentPersonal: (token, tc, fields) => request(
    "PATCH",
  ...
  updateKurumRoute: (token, route) => request(
    "PATCH",
...
// src/main/storage/student-db.ts
var import_electron3 = require("electron");
var fs3 = __toESM(require("fs"));
var path3 = __toESM(require("path"));
var FILE_NAME = "student-store.enc";
var PLAINTEXT_FALLBACK_NAME = "student-store.json";
var StudentDb = class {
  constructor() {
    this.data = { version: 2, accounts: {} };
    this.dirty = false;
    this.flushTimer = null;
    this.encPath = path3.join(import_electron3.app.getPath("userData"), FILE_NAME);
    this.plainPath = path3.join(import_electron3.app.getPath("userData"), PLAINTEXT_FALLBACK_NAME);
    this.load();
  }
  load() {
    try {
      if (fs3.existsSync(this.encPath) && import_electron3.safeStorage.isEncryptionAvailable()) {
        const buf = fs3.readFileSync(this.encPath);
        const json = import_electron3.safeStorage.decryptString(buf);
      ...
      if (fs3.existsSync(this.plainPath)) {
        const json = fs3.readFileSync(this.plainPath, "utf-8");
        const parsed = JSON.parse(json);
        if (parsed && parsed.accounts) {
          this.data = this.migrate(parsed);
  ...
  ingestDetail(accountId, data) {
    ...
    if (existing) {
      ...
      if (data.babaAd !== void 0)
        existing.babaAd = data.babaAd;
      if (data.dogumYeri !== void 0)
        existing.dogumYeri = data.dogumYeri;
      if (data.dogumTarihi !== void 0)
        existing.dogumTarihi = data.dogumTarihi;
      if (data.adres !== void 0)
        existing.adres = data.adres;
  ...
  updatePersonal(accountId, tc, fields) {
    const bucket = this.getBucket(accountId);
    const s = bucket.students[tc];
    if (!s)
      return false;
    if (fields.babaAd !== void 0)
      s.babaAd = fields.babaAd;
    if (fields.dogumYeri !== void 0)
      s.dogumYeri = fields.dogumYeri;
    if (fields.dogumTarihi !== void 0)
      s.dogumTarihi = fields.dogumTarihi;
    if (fields.adres !== void 0)
      s.adres = fields.adres;
  ...
  flush() {
    ...
    try {
      ...
      if (import_electron3.safeStorage.isEncryptionAvailable()) {
        const buf = import_electron3.safeStorage.encryptString(json);
        fs3.writeFileSync(this.encPath, buf);
        try {
          if (fs3.existsSync(this.plainPath))
            fs3.unlinkSync(this.plainPath);
        } catch {
        }
        console.log(`[StudentDb] Flushed encrypted DB (${buf.length} bytes)`);
      } else {
        fs3.writeFileSync(this.plainPath, json, "utf-8");
...
function updateStudentPersonal(tc, fields) {
  const token = tokenOrNull();
  if (!token)
    return Promise.resolve();
  return apiClient.updateStudentPersonal(token, tc, fields).then(() => {
  }).catch((e) => {
    console.error("[StudentSync] updateStudentPersonal failed:", e?.message || e);
  });
}
...
// src/main/sync/personnel-sync.ts
var _getToken2 = () => null;
...
// src/main/sync/kurum-info-sync.ts
var _getToken3 = () => null;
...
function updateKurumRoute(route) {
  ...
  if (!token)
    return Promise.resolve(null);
  return apiClient.updateKurumRoute(token, route).then((r) => r).catch((e) => {
    console.error("[KurumInfoSync] updateKurumRoute failed:", e?.message || e);
    return null;
  });
}
...
// src/launcher/remote-code-loader.ts
...
// src/launcher/desktop-crypto-client.ts
var crypto = __toESM(require("crypto"));
var https2 = __toESM(require("https"));
var http2 = __toESM(require("http"));
var import_url = require("url");
var AES_ALGO = "aes-256-gcm";
var IV_LENGTH = 12;
var TAG_LENGTH = 16;
var REQUEST_TIMEOUT_MS = 15e3;
function signRequestBody(pathToken, extra) {
  const timestamp = Date.now();
  const nonce = crypto.randomUUID();
  const signature = crypto.createHmac("sha256", DESKTOP_HMAC_SECRET).update(`${pathToken}|${timestamp}|${nonce}`).digest("hex");
  return JSON.stringify({
    ...extra || {},
    path: pathToken === "__manifest__" ? void 0 : pathToken,
    timestamp,
    nonce,
    signature
  });
}
function postSignedBinary(url, body, timeoutMs = REQUEST_TIMEOUT_MS) {
  return new Promise((resolve3, reject) => {
    const u = new import_url.URL(url);
      ...
      (res) => {
        ...
        res.on("end", () => {
          ...
          if (res.statusCode !== 200) {
            ...
            const friendlyMessage = res.statusCode === 429 ? "\xC7ok fazla istek yap\u0131ld\u0131, l\xFCtfen daha sonra tekrar deneyiniz" : res.statusCode === 404 ? "\u015Eablon bulunamad\u0131" : res.statusCode === 400 ? "Ge\xE7ersiz istek" : `Sunucu hatas\u0131 (${res.statusCode})`;
    ...
    req.on("error", (e) => {
      ...
      friendly.detail = e?.message || String(e);
      reject(friendly);
    });
    req.on("timeout", () => {
      req.destroy();
...
// src/launcher/remote-code-loader.ts
var VERSION_UPDATED_CHANNEL = "code-version-updated";
var RemoteCodeLoader = class {
  constructor() {
    ...
    this.devReadLogged = /* @__PURE__ */ new Set();
  }
  /**
   ...
   */
  readDevLocal(safePath) {
    if (!IS_DEV || FORCE_REMOTE_CODE_IN_DEV)
      return null;
    const localPath = path4.resolve(__dirname, "..", "..", "remote-code", safePath);
    if (!fs4.existsSync(localPath))
      return null;
    try {
      const buf = fs4.readFileSync(localPath);
  ...
  async sync(opts = {}) {
    ...
    try {
      ...
      let updatedCount = 0;
      const wantedPaths = /* @__PURE__ */ new Set();
      for (const [relPath, expectedHash] of Object.entries(manifest.files)) {
        ...
        const actualHash = crypto2.createHash("sha256").update(content).digest("hex");
      ...
      if (manifest.version) {
        const whatsNew = await this.fetchPlainVersion().then((v) => v.whatsNew).catch(() => void 0);
        this.setVersion(manifest.version, whatsNew);
      }
      console.log(
        updatedCount > 0 ? `[CodeLoader] Sync complete \u2014 ${updatedCount} file(s) updated. version=${manifest.version ?? "n/a"}` : `[CodeLoader] Already up to date. version=${manifest.version ?? "n/a"}`
      );
    } catch (err) {
   ...
   */
  async runScriptOrFallback(win, scriptName, fallbackJs, params = {}) {
    if (win.isDestroyed())
      return void 0;
   ...
   */
  startVersionPolling(getMainWindow) {
    if (this.pollTimer)
      return;
    if (IS_DEV && !FORCE_REMOTE_CODE_IN_DEV) {
    ...
    const intervalMs = VERSION_POLL_SECONDS * 1e3;
   ...
   */
  async showCodeWhatsNewIfPending(win) {
    ...
    if (!version || !whatsNew)
      return;
    if (this.isCodeWhatsNewDisabled())
      return;
    if (this.readLastSeenCodeVersion() === version)
      return;
    let response;
    try {
      const result = await import_electron4.dialog.showMessageBox(win, {
        type: "info",
        title: `Yenilikler \u2014 v${version}`,
        message: "Uygulama g\xFCncellendi",
        detail: whatsNew,
        buttons: ["Tamam", "Bir daha g\xF6sterme"],
        defaultId: 0,
        cancelId: 0,
        noLink: true
      });
  ...
  async pollOnce(getMainWindow) {
    if (this.restartPromptShown)
      return;
    if (!this._codeVersion)
      return;
    let latest;
    try {
    ...
    if (latest.version === this._codeVersion)
      return;
...
Bu s\xFCr\xFCmdeki yenilikler:
${latest.whatsNew}
` : "";
      await import_electron4.dialog.showMessageBox(win, {
        type: "info",
        title: "G\xFCncelleme Mevcut",
        message: "Yeni bir g\xFCncelleme y\xFCklendi",
        detail: `Devam edebilmek i\xE7in uygulaman\u0131n yeniden ba\u015Flat\u0131lmas\u0131 gerekiyor.

Mevcut s\xFCr\xFCm: v${previousVersion}
Yeni s\xFCr\xFCm: v${latest.version}` + whatsNewSection,
  ...
  fetchPlainVersion(timeoutMs = 5e3) {
    return new Promise((resolve3, reject) => {
      const u = new import_url2.URL(`${DESKTOP_CODE_BASE_URL}/version`);
      const client = u.protocol === "https:" ? https3 : http3;
      const req = client.request(
        {
          method: "GET",
          protocol: u.protocol,
  ...
  substituteParams(source, params) {
    if (!params || Object.keys(params).length === 0)
      return source;
    return source.replace(/__([A-Z0-9_]+)__/g, (match, key) => {
  ...
  lastSeenPath() {
    return path4.join(import_electron4.app.getPath("userData"), "last-seen-code-version.txt");
  }
  disabledPath() {
    return path4.join(import_electron4.app.getPath("userData"), "code-whats-new-disabled.txt");
  }
  readLastSeenCodeVersion() {
    try {
      return fs4.readFileSync(this.lastSeenPath(), "utf-8").trim();
    } catch {
      return "";
    }
  }
  writeLastSeenCodeVersion(version) {
    try {
      fs4.writeFileSync(this.lastSeenPath(), version, "utf-8");
    } catch {
    }
  }
  isCodeWhatsNewDisabled() {
    return fs4.existsSync(this.disabledPath());
  }
  setCodeWhatsNewDisabled() {
    try {
      fs4.writeFileSync(this.disabledPath(), "1", "utf-8");
    } catch {
    }
  }
};
...
function getCodeLoader() {
  if (!_instance2)
    _instance2 = new RemoteCodeLoader();
  return _instance2;
}

// src/main/utils/request-logger.ts
...
var RequestLogger = class {
  constructor() {
    ...
    const base = import_electron5.app.isPackaged ? path5.join(import_electron5.app.getPath("userData"), "logs") : path5.join(__dirname, "..", "..", "logs");
    this.logsDir = base;
    this.requestsDir = path5.join(base, "requests");
    this.responsesDir = path5.join(base, "responses");
    this.journalPath = path5.join(base, "journal.jsonl");
  ...
  async recordResponse(webContents, accountLabel, url, html) {
    ...
    const urlHash = crypto3.createHash("sha1").update(url).digest("hex").slice(0, 6);
    ...
    const reqPath = path5.join(this.requestsDir, reqFileName);
    const resPath = path5.join(this.responsesDir, resFileName);
    const reqInfo = {
      id: baseName,
      timestamp: isoTs,
    ...
    try {
      fs5.mkdirSync(this.requestsDir, { recursive: true });
      fs5.mkdirSync(this.responsesDir, { recursive: true });
      fs5.writeFileSync(reqPath, JSON.stringify(reqInfo, null, 2), "utf-8");
      fs5.writeFileSync(resPath, html, "utf-8");
      ...
      fs5.appendFileSync(this.journalPath, journalEntry + "\n", "utf-8");
    } catch (err) {
  ...
  (function() {
    ...
    function showDireksiyonModal() {
      ...
      const sinifOptions = [
        ...
        { label: 'B(2016 \xD6ncesi) \u2192 C (16 ders)', value: 'B(2016 \xD6ncesi),C|16' },
        { label: 'B(2016 Sonras\u0131) \u2192 C (22 ders)', value: 'B(2016 Sonras\u0131),C|22' },
        { label: 'B(2016 \xD6ncesi) \u2192 D (9 ders)', value: 'B(2016 \xD6ncesi),D|9' },
    ...
    // \u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500
    // K Belgesi \u2014 \xF6\u011Frenci arama ad\u0131m\u0131
    ...
    function showKBSearch(overlay, students, onSelect) {
      ...
      searchTitle.textContent = 'K Belgesi \u2014 \xD6\u011Frenci Se\xE7';
      ...
      hint.textContent = students.length
        ? 'TC veya Ad Soyad ile aray\u0131n (' + students.length + ' kay\u0131tl\u0131 \xF6\u011Frenci)'
        : 'Hen\xFCz \xF6\u011Frenci y\xFCklenmemi\u015F \u2014 \xF6nce \xD6\u011Frencileri G\xFCncelle. Manuel TC ile devam edebilirsiniz.';
      ...
      function renderResults(q) {
        ...
        if (!matches.length) {
          ...
          if (/^\\d{11}$/.test(query)) {
            ...
            fetchHint.textContent = "Bu TC yerel kay\u0131tta yok. MEBBIS'ten \xE7ekilsin mi?";
            ...
            fetchBtn.onclick = function() {
              ...
              fetchBtn.textContent = '\xC7ekiliyor...';
    ...
    function buildKBForm(modal, student, personnelList, store, overlay) {
      ...
      function shortKursAdi(full) {
        ...
        s = s.replace(/^\xD6ZEL\\s+/, '');
        s = s.replace(/\\s+(MOTORLU|TA[\u015ES]IT|S\xDCR\xDCC\xDC|KURSU)\\b.*$/, '');
        return s.trim();
      }

      function extractIlIlceFromAdres(adres) {
        if (!adres) return null;
        var m = String(adres).match(/([A-Z\xC7\u011E\u0130\xD6\u015E\xDC]+)\\s*\\/\\s*([A-Z\xC7\u011E\u0130\xD6\u015E\xDC]+)\\s*$/);
        if (!m) return null;
      ...
      // Strip trailing "\u0130L\xC7E / \u0130L" (and any preceding separator) so kurs adresi only
      // shows mahalle/cadde/no \u2014 il/il\xE7e is its own field.
      kursAdresVal = kursAdresVal.replace(/[\\s ,\\-/]*[A-Za-z\xC7\u011E\u0130\u0131\xD6\u015E\xDC\xE7\u011F\xF6\u015F\xFC]+[\\s ]*\\/[\\s ]*[A-Za-z\xC7\u011E\u0130\u0131\xD6\u015E\xDC\xE7\u011F\xF6\u015F\xFC]+[\\s .]*$/, '').trim();
      ...
      // ---- Section 1: Ara\xE7 ve \u0130zin Bilgileri ----
      var sec1 = mkSection('Ara\xE7 ve \u0130zin Bilgileri');
      mkField(sec1, 'Ara\xE7 Cinsi', 'aracCinsi', {
        type: 'select',
        options: ['Otomobil', 'Motosiklet', 'Kamyon', 'Otob\xFCs', 'Minib\xFCs', 'Kamyonet', 'Trakt\xF6r', 'Di\u011Fer']
      });
      ...
      var gurzergahInp = mkField(sec1, 'G\xFCzergah', 'gurzergah', { placeholder: 'Mahalle i\xE7i - \u015Eehir merkezi', value: gurzergahVal });
      ...
      mkField(sec1, 'G\xFCn / Saat', 'gunSaat', { placeholder: 'Pzt-Cu 09:00-17:00' });
      mkField(sec1, 'D\xFCzenlenme Tarihi', 'duzenlenmeTarihi', { type: 'date', value: todayStr });
      mkField(sec1, 'Ge\xE7erlik Biti\u015Fi', 'gecerlikBitisi', { type: 'date', value: sixMonthsStr });
      ...
      mkField(sec2, '\u0130li / \u0130l\xE7esi', 'iliIlcesi', { placeholder: 'ANKARA / \xC7ANKAYA', value: ilIlceVal });
      ...
      mkField(sec2, 'Kurs Adresi', 'kursAdresi', { placeholder: 'Mahalle, Sokak, No, \u0130l\xE7e/\u0130l', value: kursAdresVal });
      modal.appendChild(sec2);

      // ---- Section 3: S\xFCr\xFCc\xFC Aday\u0131 Bilgileri ----
      var sec3 = mkSection('S\xFCr\xFCc\xFC Aday\u0131 Bilgileri');
      ...
      var adresInp = mkField(sec3, 'Adresi', 'adayAdresi', { placeholder: 'Mahalle, No, \u0130l\xE7e/\u0130l', value: (student && student.adres) || '' });
      ...
      // ---- Section 4: Usta \xD6\u011Fretici Bilgileri ----
      var sec4 = mkSection('Usta \xD6\u011Fretici Bilgileri');
      var personnelOpts = [{ value: '', label: personnelList.length ? '\u2014 Se\xE7iniz \u2014' : '(personel bulunamad\u0131, manuel girin)' }];
      personnelList.forEach(function(p) {
        if (!p || !p.tc) return;
        personnelOpts.push({ value: p.tc, label: (p.adSoyad || ((p.ad || '') + ' ' + (p.soyad || ''))) + ' \xB7 ' + p.tc });
      });
      var personnelSelect = mkField(sec4, 'Personel Se\xE7', 'ustaSelect', { type: 'select', options: personnelOpts });
...
// src/main/mebbis/sidebar/store-sections.ts
      ...
      (function() {
        ...
        window.__mebbisRenderStore = function() {
          ...
          if (open) {
            ...
            // rebuild it directly so the "Y\xFCkleniyor..." G\xFCncelle button is
...
// src/main/mebbis/pdf-render.ts
function fetchSimulatorTemplate(m, templateName) {
...
async function generatePdfFromTemplate(m, studentInfo, lessons, sinif) {
...
function extractSimulatorSessions(m, lessons) {
...
async function generateSesimHtml(m, studentInfo, sessions, accountLabel) {
  ...
  const parseStartTime = (timeStr) => {
    ...
    const [h, m2] = startPart.split(":").map(Number);
    const d = new Date(2e3, 0, 1, h, m2, 0);
    return d;
  };
  const formatTime = (d) => {
...
function generateAnagrupHtml(m, baseTemplate, data) {
...
function getAnagrupScenarios(m) {
  return [
...
function fetchTemplate(m, templateName) {
  return fetchEncryptedTemplate(`direksiyon-takip/${templateName}`);
}
async function generateKBelgesiPdf(m, data, parentWin) {
  let html;
  try {
...
// src/main/mebbis/download-batch.ts
function trackBatchError(m, errorMessage, studentTc) {
...
function formatBatchErrorSummary(m) {
  if (!m.pendingBatchDownload || m.pendingBatchDownload.errors.size === 0)
    return "";
  const errorLines = [];
...
async function handleBatchDireksiyon(m, account, parentWin) {
  m.handleBatchGeneric("direksiyon", account, parentWin);
}
async function handleBatchGeneric(m, batchType, account, parentWin) {
...
async function handleBatchStart(m, options, account, win) {
  ...
  m.pendingBatchDownload.processedTcs = /* @__PURE__ */ new Set();
  ...
  m.pendingBatchDownload.errors = /* @__PURE__ */ new Map();
  ...
  if (isTumDonemler && m.pendingBatchDownload.donemList.length > 1) {
    const realPeriods = m.pendingBatchDownload.donemList.filter((d) => !d.label.toLowerCase().includes("t\xFCm"));
    m.pendingBatchDownload.donemList = realPeriods;
    m.pendingBatchDownload.currentDonemIndex = 0;
    if (realPeriods.length === 0) {
      m.showBatchProgress(win, "Hata: D\xF6nem listesi bo\u015F!", "#ff4444");
      m.clearPendingBatchDownload();
      m.pendingDownloadPhase = null;
      return;
    }
    const firstDonem = realPeriods[0];
    m.showBatchProgress(win, `D\xF6nem taran\u0131yor: ${firstDonem.label} (1/${realPeriods.length})...`);
    m.pendingDownloadPhase = "batch-skt02006-results";
    m.submitSkt02006Form(win, firstDonem.value, options);
  } else {
    m.pendingBatchDownload.donemList = [];
    m.showBatchProgress(win, "\xD6\u011Frenci listesi al\u0131n\u0131yor...");
    m.pendingDownloadPhase = "batch-skt02006-results";
    m.submitSkt02006Form(win, options.donemi, options);
  }
}
async function handleBatchStudentNavigate(m, win, account) {
  if (!m.pendingBatchDownload)
    return;
...
async function handleBatchStudentResults(m, win, account) {
  if (!m.pendingBatchDownload)
    return;
  ...
  const selectedDonemiLabelRaw = m.pendingBatchDownload.donemList?.find((d) => d.value === selectedDonemiValue)?.label ?? "";
...
async function batchGenerateForStudent(m, lessonData, student, account) {
  if (!m.pendingBatchDownload)
    return;
  ...
  if (batchType === "direksiyon") {
    ...
    if (simulatorSessions.length === 0) {
      ...
      m.trackBatchError("Sim\xFClat\xF6r dersi bulunamad\u0131", student.tc);
      m.pendingBatchDownload.failed++;
      return;
    }
...
async function batchProcessNextStudent(m, win, account) {
  if (!m.pendingBatchDownload)
    return;
...
function showBatchProgress(m, win, message, color) {
  if (m.pendingBatchDownload) {
...
function hideBatchStatus(m, win) {
  if (!win.isDestroyed()) {
    win.webContents.executeJavaScript(`
        (function() {
...
function reinjectBatchStatus(m, win) {
...
async function clickMenuItemForSkt02006(m, win, account) {
...
async function handleSkt02006Options(m, win, account) {
  if (!m.pendingBatchDownload)
    return;
...
async function handleSkt02006Results(m, win, account) {
  if (!m.pendingBatchDownload)
    return;
  try {
    ...
    if (m.pendingBatchDownload.donemList.length > 0 && m.pendingBatchDownload.currentDonemIndex < m.pendingBatchDownload.donemList.length - 1) {
      ...
      m.showBatchProgress(win, `D\xF6nem taran\u0131yor: ${nextDonem.label} (${m.pendingBatchDownload.currentDonemIndex + 1}/${total})... Toplam: ${m.pendingBatchDownload.students.length} \xF6\u011Frenci`);
      m.pendingDownloadPhase = "batch-skt02006-results";
      m.submitSkt02006Form(win, nextDonem.value, m.pendingBatchDownload.options);
      return;
    }
    if (m.pendingBatchDownload.statusList.length > 0 && m.pendingBatchDownload.currentStatusIndex < m.pendingBatchDownload.statusList.length - 1) {
      ...
      m.showBatchProgress(win, `Durum taran\u0131yor: ${nextStatus.label} (${m.pendingBatchDownload.currentStatusIndex + 1}/${totalStatuses})... Toplam: ${m.pendingBatchDownload.students.length} \xF6\u011Frenci`);
      m.pendingDownloadPhase = "batch-skt02006-results";
    ...
    if (totalStudents === 0) {
      m.showBatchProgress(win, "\xD6\u011Frenci bulunamad\u0131!", "#ff4444");
      m.clearPendingBatchDownload();
      m.pendingDownloadPhase = null;
      return;
    }
    console.log(`[${account.label}] Batch: total unique students: ${totalStudents}, starting PDF generation...`);
    m.showBatchProgress(win, `${totalStudents} \xF6\u011Frenci bulundu. PDF olu\u015Fturuluyor... (0/${totalStudents})`);
    m.pendingBatchDownload.currentStudentIndex = 0;
    m.pendingDownloadPhase = "batch-skt02009-navigate";
    m.clickMenuItemForSkt02009(win, account);
  } catch (e) {
    console.error(`[${account.label}] Batch: results scrape error:`, e);
    m.showBatchProgress(win, "Hata: \xD6\u011Frenci listesi okunamad\u0131", "#ff4444");
    m.clearPendingBatchDownload();
    m.pendingDownloadPhase = null;
  }
}
...
async function clickMenuItemForSkt02009(m, win, account) {
  const clicked = await win.webContents.executeJavaScript(`
      (function() {
...
async function handleSktModuleLoaded(m, win, account) {
  m.clickMenuItemForSkt02009(win, account);
}
async function handleSkt02009Loaded(m, win, account) {
  if (!m.pendingDownload || m.pendingDownload.parentWin !== win)
    return;
...
async function handleSkt02009Results(m, win, account) {
  if (!m.pendingDownload || m.pendingDownload.parentWin !== win)
    return;
...
async function handleSkt02009SimulatorLoaded(m, win, account) {
  if (!m.pendingSimulatorReport)
    return;
...
async function handleSkt02009SimulatorResults(m, win, account) {
  if (!m.pendingSimulatorReport)
    return;
...
async function handleSimulationReport(m, tc, simulationType, account, parentWin) {
...
// src/main/mebbis/student-flows.ts
function parseAndLogStudentPage(m, win, account) {
  if (win.isDestroyed())
    return;
...
function parseAndIngestStudentList(m, win, account) {
  if (win.isDestroyed())
    return;
...
function openStudent(m, win, account, tc) {
  if (win.isDestroyed()) {
...
function fillTcAndSubmit(m, win, tc) {
  if (win.isDestroyed())
    return;
  win.webContents.executeJavaScript(`
      (function() {
...
async function handleStudentUpdateOptions(m, win, account) {
  if (!m.pendingStudentUpdate.has(account.id))
    return;
  try {
    const formOptions = await win.webContents.executeJavaScript(`
        (function() {
...
async function submitStudentUpdateForm(m, win, options) {
...
function parseAndIngestPersonnelListOok(m, win, account) {
  if (win.isDestroyed())
    return;
  ...
  win.webContents.executeJavaScript(`
    ...
    if (m.personnelBatchDetailDone.has(account.id) || m.pendingPersonnelBatchDetail)
      return;
    win.webContents.executeJavaScript(`
        (function() {
...
function triggerPersonnelAcPostback(m, win, account, index) {
  if (win.isDestroyed())
    return;
...
function scrapePersonnelDetail(m, win, account) {
  if (win.isDestroyed())
    return;
  const batch = m.pendingPersonnelBatchDetail;
  if (!batch || batch.accountId !== account.id)
...
function pushStoreToSidebar(m, win, account) {
  if (win.isDestroyed())
    return;
...
var MebbisManager = class _MebbisManager {
  constructor() {
     ...
     */
    this.loginAutoFilled = /* @__PURE__ */ new Set();
  ...
  isDemoLimitReached(account) {
    ...
    const session6 = this.demoSessionUsage.get(account.id) ?? 0;
    return baseline + session6 >= _MebbisManager.DEMO_PDF_LIMIT;
  }
...
========== STARTING ACCOUNT: ${account.label} ==========`);
    ...
    win.on("closed", () => {
      ...
      const ses2 = import_electron11.session.fromPartition(partition);
    ...
    const ses = import_electron11.session.fromPartition(partition);
    ...
    win.webContents.on("did-finish-load", () => {
      ...
      if (this.isLoginPage(currentURL)) {
        if (this.loginAutoFilled.has(account.id)) {
          ...
          this.showStatus(win, "Otomatik giri\u015F ba\u015Far\u0131s\u0131z \u2014 l\xFCtfen manuel giri\u015F yap\u0131n", "#FFA500");
...
`);
  ...
  parseAndIngestPersonnelListOok(win, account) {
    return parseAndIngestPersonnelListOok(this, win, account);
  }
  /**
  ...
  triggerPersonnelAcPostback(win, account, index) {
    return triggerPersonnelAcPostback(this, win, account, index);
  }
  /**
  ...
  scrapePersonnelDetail(win, account) {
    return scrapePersonnelDetail(this, win, account);
  }
  serializeStore(account) {
    return serializeStore(this, account);
  }
  pushStoreToSidebar(win, account) {
    return pushStoreToSidebar(this, win, account);
  }
  openStudent(win, account, tc) {
    return openStudent(this, win, account, tc);
  }
  fillTcAndSubmit(win, tc) {
    return fillTcAndSubmit(this, win, tc);
  }
  async injectLeftMenu(win, account) {
    return injectLeftMenu(this, win, account);
  }
  async injectStoreSidebarSections(win, account) {
    return injectStoreSidebarSections(this, win, account);
  }
  stop(accountId) {
...
<body>
  ...
  async downloadDireksiyonTakip(tc, _partition, account, parentWin, sinif) {
    return downloadDireksiyonTakip(this, tc, _partition, account, parentWin, sinif);
  }
  async clickMenuItemForSkt02009(win, account) {
    return clickMenuItemForSkt02009(this, win, account);
  }
  async handleSktModuleLoaded(win, account) {
    return handleSktModuleLoaded(this, win, account);
  }
  async handleSkt02009Loaded(win, account) {
    return handleSkt02009Loaded(this, win, account);
  }
  async handleSkt02009Results(win, account) {
    return handleSkt02009Results(this, win, account);
  }
  async handleSkt02009SimulatorLoaded(win, account) {
    return handleSkt02009SimulatorLoaded(this, win, account);
  }
  async handleSkt02009SimulatorResults(win, account) {
    return handleSkt02009SimulatorResults(this, win, account);
  }
  fetchSimulatorTemplate(templateName) {
    return fetchSimulatorTemplate(this, templateName);
  }
  async generatePdfFromHtml(html) {
    return generatePdfFromHtml(this, html);
  }
  async generatePdfFromTemplate(studentInfo, lessons, sinif) {
    return generatePdfFromTemplate(this, studentInfo, lessons, sinif);
  }
  async handleSimulationReport(tc, simulationType, account, parentWin) {
    return handleSimulationReport(this, tc, simulationType, account, parentWin);
  }
  /**
  ...
  extractSimulatorSessions(lessons) {
    return extractSimulatorSessions(this, lessons);
  }
  /**
  ...
  async generateSimulatorReportPdf(studentInfo, simulatorSessions, simulationType, account, parentWindow) {
    return generateSimulatorReportPdf(this, studentInfo, simulatorSessions, simulationType, account, parentWindow);
  }
  /**
  ...
  async generateSesimHtml(studentInfo, sessions, accountLabel) {
    return generateSesimHtml(this, studentInfo, sessions, accountLabel);
  }
  generateAnagrupHtml(baseTemplate, data) {
    return generateAnagrupHtml(this, baseTemplate, data);
  }
  /**
   * Get list of anagrup scenarios
   */
  getAnagrupScenarios() {
    return getAnagrupScenarios(this);
  }
  fetchTemplate(templateName) {
    return fetchTemplate(this, templateName);
  }
  // ==================== BATCH DİREKSİYON TAKİP / SİMÜLATÖR ====================
  trackBatchError(errorMessage, studentTc) {
    return trackBatchError(this, errorMessage, studentTc);
  }
  formatBatchErrorSummary() {
    return formatBatchErrorSummary(this);
  }
  async handleBatchDireksiyon(account, parentWin) {
    return handleBatchDireksiyon(this, account, parentWin);
  }
  async handleBatchGeneric(batchType, account, parentWin) {
    return handleBatchGeneric(this, batchType, account, parentWin);
  }
  async clickMenuItemForSkt02006(win, account) {
    return clickMenuItemForSkt02006(this, win, account);
  }
  async handleSkt02006Options(win, account) {
    return handleSkt02006Options(this, win, account);
  }
  /**
  ...
  async handleStudentUpdateOptions(win, account) {
    return handleStudentUpdateOptions(this, win, account);
  }
  /**
  ...
  async submitStudentUpdateForm(win, options) {
    return submitStudentUpdateForm(this, win, options);
  }
  async handleBatchStart(options, account, win) {
    return handleBatchStart(this, options, account, win);
  }
  ...
  async handleSkt02006Results(win, account) {
    return handleSkt02006Results(this, win, account);
  }
  async handleBatchStudentNavigate(win, account) {
    return handleBatchStudentNavigate(this, win, account);
  }
  async handleBatchStudentResults(win, account) {
    return handleBatchStudentResults(this, win, account);
  }
  async batchGenerateForStudent(lessonData, student, account) {
    return batchGenerateForStudent(this, lessonData, student, account);
  }
  ...
  async batchProcessNextStudent(win, account) {
    return batchProcessNextStudent(this, win, account);
  }
  showBatchProgress(win, message, color) {
    return showBatchProgress(this, win, message, color);
  }
  hideBatchStatus(win) {
    return hideBatchStatus(this, win);
  }
  reinjectBatchStatus(win) {
    return reinjectBatchStatus(this, win);
  }
  ...
  async generateKBelgesiPdf(data, parentWin) {
    return generateKBelgesiPdf(this, data, parentWin);
  }
};

// src/main/app-controller.ts
var import_config7 = require("bootstrap:config");

// src/main/templates/bundle-whats-new.ts
...
function suppressLauncherWhatsNew(currentVersion) {
  try {
    fs10.writeFileSync(
      path9.join(import_electron12.app.getPath("userData"), LAUNCHER_STATE_FILE),
      currentVersion,
      "utf-8"
    );
  } catch {
...
function readBundleDismissed() {
  try {
    return fs10.readFileSync(path9.join(import_electron12.app.getPath("userData"), BUNDLE_STATE_FILE), "utf-8").trim();
  } catch {
    return "";
  }
}
function writeBundleDismissed(version) {
  try {
    fs10.writeFileSync(
      path9.join(import_electron12.app.getPath("userData"), BUNDLE_STATE_FILE),
      version,
      "utf-8"
    );
  } catch {
...
function fetchVersionWithHistory(timeoutMs = 5e3) {
  return new Promise((resolve3, reject) => {
    const u = new import_url3.URL(`${DESKTOP_CODE_BASE_URL}/version`);
    const client = u.protocol === "https:" ? https4 : http4;
    const req = client.request(
      {
        method: "GET",
        hostname: u.hostname,
...
${e.whatsNew}`).join("\n\n\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\u2500\n\n");
  let result;
  try {
    result = await import_electron12.dialog.showMessageBox(win, {
      type: "info",
...
// src/main/tests/test-pdfs.ts
var import_electron13 = require("electron");
var fs11 = __toESM(require("fs"));
async function generateTestDireksiyonPdf(m, sinif, mainWindow) {
  ...
  const saveResult = await import_electron13.dialog.showSaveDialog(mainWindow, {
  ...
  fs11.writeFileSync(saveResult.filePath, pdfBuffer);
  console.log("[DEV TEST] Saved to:", saveResult.filePath);
  await import_electron13.shell.openPath(saveResult.filePath);
}
async function generateTestKBelgesiPdf(_m, mainWindow, withBackground = false) {
...
<script>(function(){
  ...
  const previewWin = new import_electron13.BrowserWindow({
    parent: mainWindow,
    width: 900,
    height: 1180,
    title: "K Belgesi \u2014 \xD6nizleme",
...
async function generateTestSimulatorPdf(m, simType, mainWindow) {
  const fakeStudentInfo = {
    "ad-soyad": "TEST ADAY AHMET YILMAZ",
    "tc-kimlik-no": "12345678901",
    "istenen-sertifika": "B SINIFI SERT\u0130F\u0130KA (Manuel)"
  ...
  await import_electron13.shell.openPath(outputDir);
}
...
async function start(ctx) {
  ...
  function createMainWindow() {
    const win = new import_electron14.BrowserWindow({
      width: 900,
      height: 700,
      minWidth: 700,
      minHeight: 500,
    ...
    if (ctx.isDev && import_config7.OPEN_DEVTOOLS_IN_DEV) {
      win.webContents.openDevTools({ mode: "detach" });
    }
    win.on("closed", () => {
      if (mainWindow === win)
  ...
  function rebuildAppMenu() {
    ...
    import_electron14.Menu.setApplicationMenu(import_electron14.Menu.buildFromTemplate(items));
  }
  async function showAboutDialog() {
    ...
    const aboutWin = new import_electron14.BrowserWindow({
      width: 400,
      height: 440,
      resizable: false,
      minimizable: false,
    ...
    await new Promise((resolve3) => {
      ...
      const handler = (_e, action) => {
        ...
        if (!aboutWin.isDestroyed())
          aboutWin.close();
        if (action === "wa")
          import_electron14.shell.openExternal("https://wa.me/905521870334");
        else if (action === "web")
          import_electron14.shell.openExternal("https://mtsk.app");
        resolve3();
      };
  ...
  async function showDireksiyonPdfDialog() {
    if (!mainWindow || mainWindow.isDestroyed())
      return;
    const r1 = await import_electron14.dialog.showMessageBox(mainWindow, {
      type: "question",
    ...
    if (r1.response === 0)
      ...
      const r2 = await import_electron14.dialog.showMessageBox(mainWindow, {
        type: "question",
    ...
    if (!sinif)
      return;
    try {
      await generateTestDireksiyonPdf(mebbisManager, sinif, mainWindow);
    } catch (err) {
      import_electron14.dialog.showErrorBox("PDF Olu\u015Fturulamad\u0131", err?.message || "Bilinmeyen hata");
    }
  }
  async function showFakeListDialog(kind) {
    ...
    const listWin = new import_electron14.BrowserWindow({
      width: 420,
      height: 520,
      resizable: true,
      minimizable: false,
  ...
  async function showSimulatorPdfDialog() {
    if (!mainWindow || mainWindow.isDestroyed())
      return;
    const r = await import_electron14.dialog.showMessageBox(mainWindow, {
      type: "question",
    ...
    if (!simType)
      return;
    try {
      await generateTestSimulatorPdf(mebbisManager, simType, mainWindow);
    } catch (err) {
      import_electron14.dialog.showErrorBox("PDF Olu\u015Fturulamad\u0131", err?.message || "Bilinmeyen hata");
    }
  }
  async function showKBelgesiPdfDialog() {
    ...
    try {
      await generateTestKBelgesiPdf(mebbisManager, mainWindow, withBackground);
    } catch (err) {
      import_electron14.dialog.showErrorBox("K Belgesi Olu\u015Fturulamad\u0131", err?.message || "Bilinmeyen hata");
    }
  }
  function setupIPC() {
    import_electron14.ipcMain.handle("device:id", () => ctx.getDeviceId());
    import_electron14.ipcMain.handle("whats-new:check", () => ctx.getPendingWhatsNew());
    import_electron14.ipcMain.handle("whats-new:dismiss", () => {
      ctx.markWhatsNewSeen();
      return true;
    });
    import_electron14.ipcMain.handle("profile:get", async () => {
      const token = authStore.getToken();
      if (!token)
        throw new Error("Not authenticated");
      return apiClient.getProfile(token);
    });
    import_electron14.ipcMain.handle("profile:update", async (_event, phone) => {
      const token = authStore.getToken();
      if (!token)
        throw new Error("Not authenticated");
      return apiClient.updateProfile(token, phone);
    });
    import_electron14.ipcMain.handle("app:is-dev", () => ctx.isDev);
    import_electron14.ipcMain.handle("desktop-code:version", () => ctx.codeLoader.getVersion());
    import_electron14.ipcMain.handle("app:version", () => ctx.appVersion);
    import_electron14.ipcMain.handle("dev:test-direksiyon-pdf", async (_event, sinif) => {
      if (!ctx.isDev)
        throw new Error("Only available in development mode");
      if (!mainWindow)
        throw new Error("No main window");
      return generateTestDireksiyonPdf(mebbisManager, sinif || "0,B|16", mainWindow);
    });
    import_electron14.ipcMain.handle("dev:test-simulator-pdf", async (_event, simType) => {
      if (!ctx.isDev)
        throw new Error("Only available in development mode");
      if (!mainWindow)
        throw new Error("No main window");
      return generateTestSimulatorPdf(mebbisManager, simType || "sesim", mainWindow);
    });
    import_electron14.ipcMain.handle("auth:check", async () => {
      const token = authStore.getToken();
    ...
    import_electron14.ipcMain.handle("auth:login", async (_event, email, password, autoLogin) => {
      try {
    ...
    import_electron14.ipcMain.handle("auth:logout", async () => {
      const token = authStore.getToken();
      if (token) {
        try {
    ...
    import_electron14.ipcMain.handle("auth:get-saved-email", () => authStore.getSavedEmail());
    import_electron14.ipcMain.handle("auth:get-saved-school", () => authStore.getSavedSchoolName());
    import_electron14.ipcMain.handle("auth:get-saved-credentials", () => ({
      email: authStore.getSavedEmail(),
      password: authStore.getRememberedPassword(),
      autoLogin: authStore.getAutoLogin()
    }));
    import_electron14.ipcMain.handle("auth:set-auto-login", (_event, value) => {
      authStore.setAutoLogin(!!value);
      return true;
    });
    import_electron14.ipcMain.handle("auth:forgot-password", async (_event, email, phone) => {
      try {
    ...
    import_electron14.ipcMain.handle("auth:verify-reset-code", async (_event, email, code) => {
      try {
    ...
    import_electron14.ipcMain.handle("auth:reset-password", async (_event, email, code, newPassword) => {
      try {
    ...
    import_electron14.ipcMain.handle("shell:open-external", async (_event, url) => {
      if (url.startsWith("https://")) {
        await import_electron14.shell.openExternal(url);
      }
    });
    ...
    import_electron14.ipcMain.handle("accounts:list", async () => {
      const token = authStore.getToken();
      const user = authStore.getUser();
      if (!token)
        return [];
    ...
    import_electron14.ipcMain.handle("accounts:add", async (_event, data) => {
      const token = authStore.getToken();
      if (!token)
        throw new Error("Not authenticated");
    ...
    import_electron14.ipcMain.handle("accounts:update", async (_event, data) => {
      const token = authStore.getToken();
      const user = authStore.getUser();
      if (!token)
        throw new Error("Not authenticated");
    ...
    import_electron14.ipcMain.handle("accounts:remove", async (_event, id) => {
      const token = authStore.getToken();
      if (!token)
        throw new Error("Not authenticated");
      mebbisManager.stop(id);
      const partition = `persist:mebbis-${id}`;
      const ses = import_electron14.session.fromPartition(partition);
      await ses.clearStorageData();
      await apiClient.removeMebbisAccount(token, parseInt(id, 10));
      return true;
    });
    import_electron14.ipcMain.handle("accounts:start", async (_event, id) => {
      const token = authStore.getToken();
      const user = authStore.getUser();
      if (!token)
        throw new Error("Not authenticated");
    ...
    import_electron14.ipcMain.handle("accounts:local-test", async (_event, id) => {
      const token = authStore.getToken();
      if (!token)
        throw new Error("Not authenticated");
    ...
    import_electron14.ipcMain.handle("accounts:stop", async (_event, id) => {
      mebbisManager.stop(id);
      return true;
    });
    import_electron14.ipcMain.handle("accounts:focus", async (_event, id) => {
      mebbisManager.focus(id);
      return true;
    });
    import_electron14.ipcMain.handle("accounts:get-status", async (_event, id) => {
  ...
  import_electron14.app.on("before-quit", () => {
    ...
    mebbisManager.stopAll();
    try {
      require("./student-db").getStudentDb().flush();
    } catch (e) {
