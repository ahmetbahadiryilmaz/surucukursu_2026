...
async function main() {
    try {
        ...
        const schoolRepository = AppDataSource.getRepository(shared_1.DrivingSchoolEntity);
        ...
        const studentRepository = AppDataSource.getRepository(shared_1.DrivingSchoolStudentEntity);
        const now = Math.floor(Date.now() / 1000);
        for (const student of students) {
            await studentRepository.query(`INSERT INTO driving_school_students (name, email, phone, tc_number, school_id, created_at, updated_at) 
         VALUES (?, ?, ?, ?, ?, ?, ?)`, [student.name, student.email, student.phone, student.tc_number, student.school_id, now, now]);
        }
        console.log(`✅ Successfully inserted ${students.length} students`);
        console.log('\n📊 Inserted students:');
        const insertedStudents = await studentRepository.query(`SELECT id, name, tc_number FROM driving_school_students WHERE school_id = ? ORDER BY created_at DESC LIMIT 15`, [dogusSchool.id]);
...
main();
//# sourceMappingURL=seed-students-v2.js.map