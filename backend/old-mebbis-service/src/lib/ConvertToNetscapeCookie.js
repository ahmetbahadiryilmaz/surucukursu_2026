...
const convertToNetscapeCookie = (loadpath, domain = "mebbisyd.meb.gov.tr") => {
    ...
    for (const key in cookieObj) {
        lines.push(`#HttpOnly_${domain}\tFALSE\t/\tFALSE\t0\t${key}\t${cookieObj[key]}`);
    }

    fs.writeFileSync(savepath, lines.join('\n'), 'utf8');
}

module.exports = convertToNetscapeCookie