...
enum SystemLogProcessTypes {
  ...
  UPDATE_EMAIL = 6,
  UPDATE_PHONE = 7,
  // Desktop activity — defined locally in desktop-service (not in api-server enum)
  DESKTOP_ACTIVITY = 100,
}
```

**Note:** `DESKTOP_ACTIVITY = 100` is used by `desktop-service/src/activity-log/activity-log.service.ts` to log school_login and pdf_download events. It is NOT added to the api-server `enum.ts` to avoid coupling.
...
| id | int | Primary key |
| driving_school_id | int | FK to driving_schools (unique) |
| type | string | `'demo'` \| `'paid'`. Demo = blocked for now |
| pdf_print_limit | int? | Tracked limit (not enforced). Always stored as `1_000_000` |
| pdf_print_used | int | Incremented by desktop on each PDF download (tracking only) |
| ends_at | number? | Unix timestamp (seconds). `null` = never expires. Stored as TIMESTAMP via dateTransformer |
| created_at | datetime | - |
| updated_at | datetime | - |

**Active subscription logic:**
```typescript
function isSubscriptionActive(sub): boolean {
  if (!sub) return false;
...
**Trick Points:**
- `ends_at` is a Unix timestamp in **seconds** (not milliseconds). The TypeORM `dateTransformer` converts to/from JS `Date` automatically.
- `pdf_print_limit` enforcement is NOT active — it's tracking only.
- When a driving school is created, auto-create a row with `type = 'demo'` and `pdf_print_limit = 1_000_000`.
- `pdf_print_used` is incremented by the desktop-service `activity-log` endpoint on each `pdf_download` event.
